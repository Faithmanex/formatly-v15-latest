...
CREATE OR REPLACE FUNCTION increment_document_usage(user_uuid UUID)
RETURNS VOID AS $$
DECLARE
  current_month_start DATE;
  current_month_end DATE;
BEGIN
  ...
  INSERT INTO usage_stats (user_id, period_start, period_end, documents_processed, api_calls_made, storage_used_gb)
  VALUES (user_uuid, current_month_start, current_month_end, 1, 0, 0.00)
...
CREATE OR REPLACE FUNCTION update_storage_usage(user_uuid UUID, storage_gb DECIMAL(10,2))
RETURNS VOID AS $$
DECLARE
  current_month_start DATE;
  current_month_end DATE;
BEGIN
  ...
  INSERT INTO usage_stats (user_id, period_start, period_end, documents_processed, api_calls_made, storage_used_gb)
  VALUES (user_uuid, current_month_start, current_month_end, 0, 0, storage_gb)
  ON CONFLICT (user_id, period_start, period_end)
  DO UPDATE SET 
